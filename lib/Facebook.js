...
function Facebookbot(configuration) {
    ...
    var facebook_api = {
        user_profile: function (psid, fields, cb) {
            if (typeof fields === "function") {
                cb = fields;
                fields = "first_name,last_name,profile_pic,locale,timezone,gender,is_payment_enabled,last_ad_referral";
            }
            request.get('https://' + api_host + '/v2.6/' + psid + '?fields=' + fields + '&access_token=' + configuration.access_token,
                function (err, res, body) {
                    if (err) {
                        facebook_botkit.log('Could not get user profile');
                        cb(err);
                    } else {
                        facebook_botkit.debug('Successfully got user profile ', body);
                        try {
                            body = JSON.parse(body);
                        } catch (ex) {
                            facebook_botkit.log('bad json received from facebook for user profile', ex);
                            return cb(ex);
                        }
                        cb(null, body);
                    }
    ...
    facebook_botkit.defineBot(function (botkit, config) {
        ...
        bot.send = function (message, cb) {
            ...
            if (message.user_ref) {
                facebook_message.recipient.user_ref = message.user_ref;
            } else if (typeof (message.channel) == 'string' && message.channel.match(/\+\d+\(\d\d\d\)\d\d\d\-\d\d\d\d/)) {
            ...
            for (var e = 0; e < obj.entry.length; e++) {
                for (var m = 0; m < obj.entry[e].messaging.length; m++) {
                    ...
                    if (facebook_message.message) {
                        ...
                        var senderId = facebook_message.sender ? facebook_message.sender.id : "";
                        var message = {
